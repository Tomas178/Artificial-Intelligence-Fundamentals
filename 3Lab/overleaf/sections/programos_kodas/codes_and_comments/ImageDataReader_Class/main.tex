...
\begin{enumerate}
    ...
    \item \texttt{\_reduce\_dataset} atsitiktinai parenka \texttt{subset\_ratio} dalį indeksų ir grąžina \texttt{torch.utils.data.Subset}. Naudojamas atskiras \texttt{torch.Generator} su \texttt{seed = 42}, todėl poaibis kiekvieną kartą yra tas pats.
    \item \texttt{read} paima visus tris \texttt{ImageFolder} aplankus (\textit{train}, \textit{validation}, \textit{test}) ir grąžina jų poaibius.
\end{enumerate}
