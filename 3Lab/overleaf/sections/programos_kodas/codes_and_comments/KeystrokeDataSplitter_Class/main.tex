...
Klasė \texttt{KeystrokeDataSplitter} iš pradinio CMU CSV failo paruošia \textit{train}, \textit{validation} ir \textit{test} aibes: pasirenka subjektus, normalizuoja požymius ir grupuotai padalija duomenis (žr.~\ref{fig:keystroke_data_splitter_class_first} ir \ref{fig:keystroke_data_splitter_class_last} pav.).
...
\begin{enumerate}
    ...
    \item Iš likusių dalyvių pagal abėcėlę atrenkami pirmieji $30$ subjektai – gauname $30$ klasių klasifikavimo užduotį su $1\,500$ įrašų.
    ...
    \item Atliekamas dvigubas grupuotas dalijimas (\texttt{stratify=y}): pirmas – $80\ \%$ \textit{train+val} ir $20\ \%$ \textit{test}; antras – iš \textit{train+val} aibės dar $80\,\%$ / $20\,\%$ į \textit{train} ir \textit{validation}. Bendras santykis – $64{:}16{:}20$.
